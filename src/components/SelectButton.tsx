...
export const SelectButton: React.FC<SelectButtonProps> = ({ character, onSelect }) => {
  ...
  return (
    <motion.button
      ...
      className="relative group overflow-hidden px-5 sm:px-7 py-3 sm:py-3.5 rounded-full border border-white/70 hover:border-white backdrop-blur-2xl bg-white/20 hover:bg-white/35 transition-all duration-300 cursor-pointer flex items-center justify-center gap-2 shadow-[0_8px_30px_rgba(0,0,0,0.3)]"
      style={{
        ...
        <AnimatePresence mode="wait">
          {selected ? (
            ...
            >
              ...
              <span className="text-white font-extrabold">SELECTED</span>
            </motion.div>
          ) : (
            <motion.div
              key="default-state"
